...
DEAL_II_NAMESPACE_OPEN namespace VectorTools
{
  ...
  namespace internal
  {
    ...
    template < int dim, int spacedim, typename FEValuesType, typename T3 >
      void
      transform (const typename FiniteElementData <
		 dim >::Conformity conformity, const unsigned int offset,
    ...
    template < int dim,
      ...
      interpolate (const Mapping < dim, spacedim > &mapping,
		   ...
		   T & function,
		   VectorType & vec, const ComponentMask & component_mask)
    {
      Assert (component_mask.
	      represents_n_components (dof_handler.get_fe ().n_components ()),
	      ExcMessage
      ...
      Assert (component_mask.
	      n_selected_components (dof_handler.get_fe ().n_components ()) >
	      0, ComponentMask::ExcNoComponentSelected ());
	...
	spacedim > &fe (dof_handler.get_fe_collection ());
	...
	number >>> fe_function_values (fe.size ());
    ...
    for (auto cell:dof_handler.active_cell_iterators ())
	{
	  ...
	  const std::vector < Point <
	    spacedim >> &generalized_support_points =
	  ...
	  FETools::
	    convert_generalized_support_point_values_to_dof_values (fe
								    [fe_index],
								    function_values,
								    dof_values);
	  ...
	  for (unsigned int i = 0; i < n_dofs; ++i)
	      ...
	      if (selected)
		{
		  ...
		  ::dealii::internal::ElementAccess <
		    VectorType >::add (dof_values[i], dofs_on_cell[i],
				       interpolation);
		  ::dealii::internal::ElementAccess <
		    VectorType >::add (typename VectorType::value_type (1.0),
				       dofs_on_cell[i], weights);
		}
	      else
		{
		  ...
		  if (locally_owned_dofs.is_element (dofs_on_cell[i]))
		    {
		      const auto value =::dealii::internal::ElementAccess <
			VectorType >::get (vec, dofs_on_cell[i]);
		      ::dealii::internal::ElementAccess <
			VectorType >::add (value, dofs_on_cell[i],
					   interpolation);
		      ::dealii::internal::ElementAccess <
			VectorType >::add (typename VectorType::
					   value_type (1.0), dofs_on_cell[i],
					   weights);
		    }
		}
	    }
	...
	{
	  const auto value =::dealii::internal::ElementAccess <
	    VectorType >::get (interpolation,
			       i);
	  const auto weight =::dealii::internal::ElementAccess <
	    VectorType >::get (weights, i);
	  ::dealii::internal::ElementAccess <
	    VectorType >::set (value / weight, i, vec);
	}
  ...
  template < int dim,
    ...
    interpolate (const Mapping < dim, spacedim > &mapping,
		 ...
		 typename VectorType::value_type > &function,
		 VectorType & vec, const ComponentMask & component_mask)
  {
    ...
    const auto function_map =
      [&function] (const typename DoFHandlerType < dim,
		   spacedim >::active_cell_iterator &)->const Function <
      spacedim, typename VectorType::value_type > *
    {
      return &function;
    };

    internal::interpolate (mapping, dof_handler, function_map, vec,
			   component_mask);
  }
		 ...
		 VectorType & vec, const ComponentMask & component_mask)
  {
    interpolate (StaticMappingQ1 < dim, spacedim >::mapping,
		 dof, function, vec, component_mask);
  }
  ...
  template < int dim, class InVector, class OutVector, int spacedim >
    ...
    interpolate (const DoFHandler < dim, spacedim > &dof_1,
		 ...
		 const InVector & data_1, OutVector & data_2)
  {
	...
	for (unsigned int j = 0; j < dof_2.get_fe ().dofs_per_cell; ++j)
	  {
	    ::dealii::internal::ElementAccess <
	      OutVector >::add (cell_data_2 (j), local_dof_indices[j],
				data_2);
	    ...
	    Assert (touch_count[local_dof_indices[j]] <
		    std::numeric_limits <
		    decltype (touch_count)::value_type >::max (),
		    ExcInternalError ());
	...
	const value_type val =::dealii::internal::ElementAccess <
	  OutVector >::get (data_2, i);

	::dealii::internal::ElementAccess <
	  OutVector >::set (val / static_cast < value_type > (touch_count[i]),
			    i, data_2);
      }
  }
  ...
  template < int dim,
    ...
    template < int, int >class DoFHandlerType >
    void
    interpolate_based_on_material_id (const Mapping < dim,
				      spacedim > &mapping,
				      const DoFHandlerType < dim,
				      spacedim > &dof_handler,
				      const std::map < types::material_id,
				      const Function < spacedim,
				      typename VectorType::value_type >
				      *>&functions, VectorType & vec,
				      const ComponentMask & component_mask)
  {
    // Create a small lambda capture wrapping the function map and call the
    // internal implementation
    const auto function_map =
      [&functions] (const typename DoFHandlerType < dim,
		    spacedim >::active_cell_iterator & cell)->const Function <
      spacedim, typename VectorType::value_type > *
    {
  ...
  namespace internal
  {
    ...
    template < int dim,
      ...
      typename number >
      void
      interpolate_zero_boundary_values (const DoFHandlerType < dim,
					spacedim > &dof_handler,
					std::map < types::global_dof_index,
					number > &boundary_values)
    {
  ...
  template < int dim,
    ...
    template < int, int >class DoFHandlerType >
    void
    interpolate_to_different_mesh (const DoFHandlerType < dim,
				   spacedim > &dof1, const VectorType & u1,
				   const DoFHandlerType < dim,
				   spacedim > &dof2, VectorType & u2)
  {
  ...
  template < int dim,
    ...
    template < int, int >class DoFHandlerType >
    void
    interpolate_to_different_mesh (const InterGridMap < DoFHandlerType < dim,
				   spacedim >> &intergridmap,
				   const VectorType & u1,
				   const AffineConstraints <
				   typename VectorType::value_type >
				   &constraints, VectorType & u2)
  {
    ...
    // grid ..
    typename DoFHandlerType < dim, spacedim >::cell_iterator cell1 =
      dof1.begin ();
  ...
  namespace internal
  {
    ...
    template < typename number >
      bool
      constraints_and_b_v_are_compatible (const AffineConstraints < number >
					  &constraints,
					  std::map < types::global_dof_index,
					  number > &boundary_values)
    {
      for (typename std::map < types::global_dof_index,
	   number >::iterator it = boundary_values.begin ();
    ...
    template < int dim,
      ...
      template < int >class Q_or_QC >
      void
      do_project (const M_or_MC < dim, spacedim > &mapping,
		  const DoFHandlerType < dim, spacedim > &dof,
		  const AffineConstraints < typename VectorType::value_type >
		  &constraints, const Q_or_QC < dim > &quadrature,
		  const Function < spacedim,
		  typename VectorType::value_type > &function,
		  VectorType & vec_result, const bool enforce_zero_boundary,
		  const Q_or_QC < dim - 1 > &q_boundary,
		  const bool project_to_boundary_first)
    {
      using number = typename VectorType::value_type;
      ...
      project_compute_b_v (mapping,
			   ...
			   q_boundary,
			   project_to_boundary_first, boundary_values);
      ...
      if (constraints_are_compatible)
	{
	  ...
	  MatrixCreator::create_mass_matrix (mapping,
					     ...
					     mass_matrix,
					     function,
					     tmp, dummy, constraints);
	  if (boundary_values.size () > 0)
	    MatrixTools::apply_boundary_values (boundary_values, mass_matrix,
						vec, tmp, true);
	}
      else
	{
    ...
    template < int components,
      ...
      int spacedim >
      void
      project_matrix_free (const Mapping < dim, spacedim > &mapping,
			   ...
			   Number >::value_type > &function,
			   LinearAlgebra::distributed::Vector < Number >
			   &work_result, const bool enforce_zero_boundary,
	...
	(update_values | update_JxW_values);
      std::shared_ptr < MatrixFree < dim,
	Number >> matrix_free (new MatrixFree < dim, Number > ());
      matrix_free->reinit (mapping, dof, constraints,
			   QGauss < 1 > (dof.get_fe ().degree + 2),
			   additional_data);
      using MatrixType =
	MatrixFreeOperators::MassOperator < dim, fe_degree, fe_degree + 2,
	components, LinearAlgebra::distributed::Vector < Number >>;
	...
	FEEvaluation < dim, fe_degree, fe_degree + 2, components,
	  Number > phi (*matrix_free);
	for (unsigned int cell = 0; cell < matrix_free->n_macro_cells ();
	     ++cell)
	  {
	    phi.reinit (cell);
    ...
    template < int components, int dim, typename Number, int spacedim >
      void
      project_matrix_free_degree (const Mapping < dim, spacedim > &mapping,
				  const DoFHandler < dim, spacedim > &dof,
				  ...
				  Vector < Number >::value_type > &function,
    ...
    template < int dim, typename Number, int spacedim >
      void
      project_matrix_free_component (const Mapping < dim, spacedim > &mapping,
				     const DoFHandler < dim, spacedim > &dof,
				     const AffineConstraints < Number >
				     &constraints,
				     const Quadrature < dim > &quadrature,
    ...
    template < int dim, typename VectorType, int spacedim >
      void
      project_matrix_free_copy_vector (const Mapping < dim,
				       spacedim > &mapping,
				       ...
				       const Function < spacedim,
				       typename VectorType::value_type >
				       &function, VectorType & vec_result,
				       const bool enforce_zero_boundary,
	...
	work_result;
      project_matrix_free_component (mapping,
				     ...
				     work_result,
				     enforce_zero_boundary,
				     q_boundary, project_to_boundary_first);
	...
	::dealii::internal::ElementAccess <
	  VectorType >::set (work_result (*it), *it, vec_result);
    ...
    template < typename VectorType, int dim >
      ...
      project (const Mapping < dim > &mapping,
	       ...
	       const AffineConstraints < typename VectorType::value_type >
	       &constraints, const Quadrature < dim > &quadrature,
	       const Function < dim,
	       typename VectorType::value_type > &function,
	       VectorType & vec_result, const bool enforce_zero_boundary,
	       const Quadrature < dim - 1 > &q_boundary,
	...
	MatrixFree < dim,
	typename VectorType::value_type >::is_supported (dof.get_fe ());
      ...
      else
	{
	  Assert ((dynamic_cast < const parallel::Triangulation < dim >
		  ...
		  ExcNotImplemented ());
	  do_project (mapping, dof, constraints, quadrature, function,
    ...
    template < int dim, typename VectorType, int spacedim, int fe_degree >
      void
      project_parallel (const Mapping < dim, spacedim > &mapping,
			const DoFHandler < dim, spacedim > &dof,
			const AffineConstraints <
			...
			const std::function <
			typename VectorType::
			value_type (const typename DoFHandler < dim,
	...
	Number >> matrix_free (new MatrixFree < dim, Number > ());
      matrix_free->reinit (mapping, dof, constraints,
			   ...
			   additional_data);
      using MatrixType =
	MatrixFreeOperators::MassOperator < dim, fe_degree, fe_degree + 2, 1,
	LinearAlgebra::distributed::Vector < Number >>;
      MatrixType mass_matrix;
      mass_matrix.initialize (matrix_free);
      mass_matrix.compute_diagonal ();
	  ...
	  if (cell->is_locally_owned ())
	      ...
	      for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
		{
		  ...
		  for (unsigned int i = 0; i < dofs_per_cell; ++i)
		    cell_rhs (i) +=
		      (fe_values.shape_value (i, q_point) * val_q *
    ...
    template < int dim,
      ...
      int n_q_points_1d >
      void
      project_parallel (std::shared_ptr < const MatrixFree < dim,
			typename VectorType::value_type >> matrix_free,
			const AffineConstraints <
			typename VectorType::value_type > &constraints,
			const std::function < VectorizedArray <
			typename VectorType::value_type > (const unsigned int,
							   const unsigned int)
			>&func, VectorType & vec_result,
	...
	dim,
	fe_degree,
	n_q_points_1d, 1, LinearAlgebra::distributed::Vector < Number >>;
      MatrixType mass_matrix;
      mass_matrix.initialize (matrix_free,
			      {
			      fe_component});
      ...
      // assemble right hand side:
      {
	FEEvaluation < dim, fe_degree, n_q_points_1d, 1,
	  Number > fe_eval (*matrix_free, fe_component);
  ...
  template < int dim, typename VectorType, int spacedim >
    ...
    project (const Mapping < dim, spacedim > &mapping,
	     ...
	     &constraints, const Quadrature < dim > &quadrature,
	     const std::function <
	     typename VectorType::value_type (const typename DoFHandler < dim,
					      spacedim >::
	     ...
	     VectorType & vec_result)
  {
    switch (dof.get_fe ().degree)
      {
      case 1:
	internal::project_parallel < dim, VectorType, spacedim, 1 > (mapping,
								     dof,
								     constraints,
								     quadrature,
								     func,
	...
	break;
      case 2:
	internal::project_parallel < dim, VectorType, spacedim, 2 > (mapping,
								     dof,
								     constraints,
								     quadrature,
								     func,
	...
	break;
      case 3:
	internal::project_parallel < dim, VectorType, spacedim, 3 > (mapping,
								     dof,
								     constraints,
								     quadrature,
								     func,
  ...
  template < int dim, typename VectorType >
    ...
    project (std::shared_ptr <
	     ...
	     typename VectorType::value_type >> matrix_free,
	     const AffineConstraints < typename VectorType::value_type >
	     &constraints, const unsigned int n_q_points_1d,
	     const std::function < VectorizedArray <
	     typename VectorType::value_type > (const unsigned int,
						const unsigned int) >&func,
	     VectorType & vec_result, const unsigned int fe_component)
  {
	...
	{
	case 1:
	  internal::project_parallel < dim, VectorType, dim, 1,
	    2 > (matrix_free, constraints, func, vec_result, fe_component);
	  break;
	case 2:
	  internal::project_parallel < dim, VectorType, dim, 2,
	    3 > (matrix_free, constraints, func, vec_result, fe_component);
	  break;
	case 3:
	  internal::project_parallel < dim, VectorType, dim, 3,
	    4 > (matrix_free, constraints, func, vec_result, fe_component);
	  break;
	default:
	  internal::project_parallel < dim, VectorType, dim, -1,
	    0 > (matrix_free, constraints, func, vec_result, fe_component);
	}
    else
      internal::project_parallel < dim, VectorType, dim, -1, 0 > (matrix_free,
								  constraints,
								  func,
								  vec_result,
								  fe_component);
	     ...
	     const bool project_to_boundary_first)
  {
	...
	const Function < dim,
	  ...
	  dynamic_cast < const Function < dim,
	  typename VectorType::value_type > *>(&function);
	...
	internal::project < VectorType, dim > (*mapping_ptr,
      ...
      {
	Assert ((dynamic_cast < const parallel::Triangulation < dim,
		 spacedim > *>(&(dof.get_triangulation ())) == nullptr),
		ExcNotImplemented ());
	internal::do_project (mapping, dof, constraints, quadrature, function,
			      vec_result, enforce_zero_boundary, q_boundary,
			      project_to_boundary_first);
      }
  }
  ...
  template < int dim, typename VectorType, int spacedim >
    ...
    project (const DoFHandler < dim, spacedim > &dof,
	     ...
	     &constraints, const Quadrature < dim > &quadrature,
	     const Function < spacedim,
	     typename VectorType::value_type > &function, VectorType & vec,
	     const bool enforce_zero_boundary,
	     const Quadrature < dim - 1 > &q_boundary,
	     const bool project_to_boundary_first)
  {
  ...
  template < int dim, typename VectorType, int spacedim >
    ...
    project (const hp::MappingCollection < dim, spacedim > &mapping,
	     ...
	     const AffineConstraints < typename VectorType::value_type >
	     &constraints, const hp::QCollection < dim > &quadrature,
	     const Function < spacedim,
	     typename VectorType::value_type > &function,
	     VectorType & vec_result, const bool enforce_zero_boundary,
  ...
  template < int dim, int spacedim, typename VectorType >
    void
    create_right_hand_side (const Mapping < dim, spacedim > &mapping,
			    const DoFHandler < dim, spacedim > &dof_handler,
			    const Quadrature < dim > &quadrature,
			    const Function < spacedim,
			    typename VectorType::value_type > &rhs_function,
			    ...
			    typename VectorType::value_type > &constraints)
  {
    ...
    UpdateFlags update_flags =
      UpdateFlags (update_values | update_quadrature_points |
		   update_JxW_values);
    FEValues < dim, spacedim > fe_values (mapping, fe, quadrature,
					  update_flags);
	  ...
	  if (cell->is_locally_owned ())
	    {
	      ...
	      const std::vector < double >&weights =
		fe_values.get_JxW_values ();
	      rhs_function.vector_value_list (fe_values.
  ...
  template < int dim, int spacedim, typename VectorType >
    void
    create_right_hand_side (const DoFHandler < dim, spacedim > &dof_handler,
			    const Quadrature < dim > &quadrature,
			    const Function < spacedim,
			    typename VectorType::value_type > &rhs_function,
			    VectorType & rhs_vector,
  ...
  template < int dim, int spacedim, typename VectorType >
    void
    create_right_hand_side (const hp::MappingCollection < dim,
			    spacedim > &mapping, const hp::DoFHandler < dim,
			    spacedim > &dof_handler,
			    ...
			    typename VectorType::value_type > &constraints)
  {
    ...
    UpdateFlags update_flags =
      UpdateFlags (update_values | update_quadrature_points |
		   update_JxW_values);
    hp::FEValues < dim, spacedim > x_fe_values (mapping, fe, quadrature,
						update_flags);
		  ...
		  for (unsigned int point = 0; point < n_q_points; ++point)
		    for (unsigned int i = 0; i < dofs_per_cell; ++i)
		      for (unsigned int comp_i = 0; comp_i < n_components;
			   ++comp_i)
			if (cell->get_fe ().
			    get_nonzero_components (i)[comp_i])
			  {
			    cell_vector (i) +=
			      rhs_values[point] (comp_i) *
			      fe_values.shape_value_component (i,
  ...
  template < int dim, int spacedim, typename VectorType >
    void
    create_right_hand_side (const hp::DoFHandler < dim,
			    spacedim > &dof_handler,
			    const hp::QCollection < dim > &quadrature,
			    const Function < spacedim,
			    typename VectorType::value_type > &rhs_function,
			    ...
			    typename VectorType::value_type > &constraints)
  {
    create_right_hand_side (hp::StaticMappingQ1 < dim,
			    spacedim >::mapping_collection, dof_handler,
				...
				Vector < double >&rhs_vector)
  {
    ...
    Quadrature < dim > q (GeometryInfo <
			  dim >::project_to_unit_cell (cell_point.second));
  ...
  template < int dim, int spacedim >
    void
    create_point_source_vector (const DoFHandler < dim,
				spacedim > &dof_handler,
				const Point < spacedim > &p,
				Vector < double >&rhs_vector)
  {
    create_point_source_vector (StaticMappingQ1 < dim, spacedim >::mapping,
				dof_handler, p, rhs_vector);
  }


  template < int dim, int spacedim >
    void
    create_point_source_vector (const hp::MappingCollection < dim,
				spacedim > &mapping,
				...
				Vector < double >&rhs_vector)
  {
    ...
    std::pair < typename hp::DoFHandler < dim,
      spacedim >::active_cell_iterator, Point < spacedim >> cell_point =
      GridTools::find_active_cell_around_point (mapping, dof_handler, p);
    ...
    const unsigned int dofs_per_cell =
      cell_point.first->get_fe ().dofs_per_cell;
  ...
  template < int dim, int spacedim >
    void
    create_point_source_vector (const hp::DoFHandler < dim,
				spacedim > &dof_handler,
				const Point < spacedim > &p,
				Vector < double >&rhs_vector)
  {
    create_point_source_vector (hp::StaticMappingQ1 <
				dim >::mapping_collection, dof_handler, p,
				rhs_vector);
  }
				...
				Vector < double >&rhs_vector)
  {
    ...
    const std::pair < typename DoFHandler < dim,
      spacedim >::active_cell_iterator, Point < spacedim >> cell_point =
      GridTools::find_active_cell_around_point (mapping, dof_handler, p);

    const Quadrature < dim > q (GeometryInfo <
				dim >::project_to_unit_cell (cell_point.
							     second));
  ...
  {
    create_point_source_vector (StaticMappingQ1 < dim, spacedim >::mapping,
				dof_handler, p, orientation, rhs_vector);
  }
  ...
  template < int dim, int spacedim, typename VectorType >
    ...
    create_boundary_right_hand_side (const Mapping < dim, spacedim > &mapping,
				     ...
				     const Function < spacedim,
				     typename VectorType::value_type >
				     &rhs_function, VectorType & rhs_vector,
				     ...
				     &boundary_ids)
  {
	...
	for (; cell != endc; ++cell)
	  for (unsigned int face = 0;
		...
		&& (boundary_ids.empty ()
		    || (boundary_ids.
			find (cell->face (face)->boundary_id ()) !=
			boundary_ids.end ())))
	      {
  ...
  template < int dim, int spacedim, typename VectorType >
    void
    create_boundary_right_hand_side (const DoFHandler < dim,
				     spacedim > &dof_handler,
				     const Quadrature < dim - 1 > &quadrature,
				     const Function < spacedim,
				     typename VectorType::value_type >
				     ...
				     &boundary_ids)
  {
    create_boundary_right_hand_side (StaticMappingQ1 < dim >::mapping,
				     dof_handler,
				     quadrature,
				     rhs_function, rhs_vector, boundary_ids);
  }
  ...
  template < int dim, int spacedim, typename VectorType >
    void
    create_boundary_right_hand_side (const hp::MappingCollection < dim,
				     spacedim > &mapping,
				     const hp::DoFHandler < dim,
				     spacedim > &dof_handler,
				     const hp::QCollection < dim - 1 >
				     &quadrature, const Function < spacedim,
				     typename VectorType::value_type >
				     &rhs_function, VectorType & rhs_vector,
				     const std::set < types::boundary_id >
				     &boundary_ids)
  {
    ...
    UpdateFlags update_flags =
      UpdateFlags (update_values | update_quadrature_points |
		   update_JxW_values);
    hp::FEFaceValues < dim > x_fe_values (mapping, fe, quadrature,
					  update_flags);
  ...
  template < int dim, int spacedim, typename VectorType >
    void
    create_boundary_right_hand_side (const hp::DoFHandler < dim,
				     spacedim > &dof_handler,
				     const hp::QCollection < dim - 1 >
				     &quadrature, const Function < spacedim,
				     typename VectorType::value_type >
				     ...
				     &boundary_ids)
  {
    create_boundary_right_hand_side (hp::StaticMappingQ1 <
				     dim >::mapping_collection, dof_handler,
				     quadrature, rhs_function, rhs_vector,
				     boundary_ids);
  }
  ...
  namespace internal
  {
    template < int dim,
      ...
      template < int, int >class M_or_MC >
      static inline void
      do_interpolate_boundary_values (const M_or_MC < dim,
				      spacedim > &mapping,
				      const DoFHandlerType < dim,
				      spacedim > &dof,
    ...
    {
      Assert (component_mask.
	      represents_n_components (dof.get_fe (0).n_components ()),
	      ExcMessage
	      ("The number of components in the mask has to be either "
	       "zero or equal to the number of components in the finite "
	       "element."));
	...
	return;
      ...
      for (typename std::map < types::boundary_id,
	   ...
	   i = function_map.begin (); i != function_map.end (); ++i)
	  ...
	  for (typename DoFHandlerType < dim, spacedim >::active_cell_iterator
		   ...
		   function_map.end ()))
		{
		  const Function < spacedim, number > &boundary_function =
		    *function_map.find (cell->face (direction)->
					boundary_id ())->second;
		  ...
		  if (fe.n_components () == 1)
		    function_values (0) =
		      ...
		      boundary_function.vector_value (cell->
						      vertex (direction),
						      function_values);
      ...
      else			// dim > 1
	{
	  ...
	  for (unsigned int f = 0; f < finite_elements.size (); ++f)
	      ...
	      else
		{
		  ...
		  std::vector < Point <
		    dim - 1 >> unit_support_points (fe.dofs_per_face);
	  ...
	  dealii::hp::FEFaceValues < dim,
	    spacedim > x_fe_values (mapping_collection, finite_elements,
				    q_collection, update_quadrature_points);
	  ...
	  for (; cell != endc; ++cell)
		   ...
		   face_no < GeometryInfo < dim >::faces_per_cell; ++face_no)
		{
		  ...
		  const typename DoFHandlerType < dim,
		    spacedim >::face_iterator face = cell->face (face_no);
		  ...
		  if ((function_map.find (boundary_component) !=
		      ...
		      if (fe_is_system)
			{
			  ...
			  function_map.find (boundary_component)->second->
			    vector_value_list (dof_locations,
					       dof_values_system);
			      ...
			      else
				{
				  ...
				  if (!fe.is_primitive (cell_i))
				    for (unsigned int c = 0; c < n_components;
					 ++c)
				      if (fe.
					  get_nonzero_components (cell_i)[c])
				  ...
				  // we will use
				  component =
				    fe.get_nonzero_components (cell_i).
  ...
  {
    internal::do_interpolate_boundary_values (mapping, dof, function_map,
					      boundary_values,
					      component_mask_);
  }
  ...
  template < int dim, int spacedim, typename number >
    void
    interpolate_boundary_values (const hp::MappingCollection < dim,
				 spacedim > &mapping,
				 const hp::DoFHandler < dim, spacedim > &dof,
				 const std::map < types::boundary_id,
				 const Function < spacedim,
  ...
  template < int dim,
    ...
    typename number >
    void
    interpolate_boundary_values (const DoFHandlerType < dim, spacedim > &dof,
				 const types::boundary_id boundary_component,
				 const Function < spacedim,
				 number > &boundary_function,
				 std::map < types::global_dof_index,
				 ...
				 const ComponentMask & component_mask_)
  {
    ...
    typename std::map < types::global_dof_index,
      number >::const_iterator boundary_value = boundary_values.begin ();
				 ...
				 const ComponentMask & component_mask)
  {
    interpolate_boundary_values (StaticMappingQ1 < dim, spacedim >::mapping,
				 dof,
				 function_map, constraints, component_mask);
  }
  ...
  namespace internal
  {
    ...
    template < typename number1, typename number2 >
      bool real_part_bigger_than (const number1 a, const number2 & b)
    {
      return a > b;
    }
    ...
    template < typename number1, typename number2 >
      bool
      real_part_bigger_than (const number1 a,
			     const std::complex < number2 > b)
    {
      Assert (std::abs (b.imag ()) <= 1e-15 * std::abs (b),
	      ExcInternalError ());
      return a > b.real ();
    }
    ...
    template < typename number1, typename number2 >
      ...
      real_part_bigger_than (const std::complex < number1 > a,
			     const number2 b)
    {
      Assert (std::abs (a.imag ()) <= 1e-15 * std::abs (a),
	      ExcInternalError ());
      return a.real () > b;
    }
    ...
    template < typename number >
      void
      invert_mass_matrix (const SparseMatrix < number > &mass_matrix,
			  const FilteredMatrix < Vector <
			  number >> &filtered_mass_matrix,
			  FilteredMatrix < Vector <
			  number >> &filtered_preconditioner,
			  const Vector < number > &rhs,
			  Vector < number > &boundary_projection)
    {
      ...
      cg.solve (filtered_mass_matrix,
		boundary_projection, rhs, filtered_preconditioner);
	...
	Vector < number >
	  boundary_projection_part (boundary_projection.size ());
	invert_mass_matrix (mass_matrix, filtered_mass_matrix,
			    filtered_preconditioner, rhs_part,
      ...
      if (dim >= 3)
	{
      ...
      MatrixCreator::create_boundary_mass_matrix (mapping,
						  dof,
						  q,
						  mass_matrix,
						  boundary_functions,
      ...
      FilteredMatrix < Vector < number >> filtered_mass_matrix (mass_matrix,
								true);
  ...
  {
    internal::do_project_boundary_values (mapping, dof, boundary_functions, q,
			     ...
			     std::vector < unsigned int >component_mapping)
  {
    project_boundary_values (StaticMappingQ1 < dim, spacedim >::mapping,
			     dof,
			     boundary_functions,
			     q, boundary_values, component_mapping);
  }
  ...
  template < int dim, int spacedim, typename number >
    void
    project_boundary_values (const hp::MappingCollection < dim,
			     spacedim > &mapping, const hp::DoFHandler < dim,
			     spacedim > &dof,
			     const std::map < types::boundary_id,
			     const Function < spacedim,
  ...
  template < int dim, int spacedim, typename number >
    void
    project_boundary_values (const hp::DoFHandler < dim, spacedim > &dof,
			     const std::map < types::boundary_id,
			     const Function < spacedim,
			     number > *>&boundary_function,
			     const hp::QCollection < dim - 1 > &q,
			     ...
			     std::vector < unsigned int >component_mapping)
  {
    project_boundary_values (hp::StaticMappingQ1 < dim,
			     spacedim >::mapping_collection, dof,
			     boundary_function, q, boundary_values,
			     component_mapping);
  }
			     ...
			     std::vector < unsigned int >component_mapping)
  {
    project_boundary_values (StaticMappingQ1 < dim, spacedim >::mapping,
			     dof,
			     boundary_functions,
			     q, constraints, component_mapping);
  }
  ...
  namespace internal
  {
     ...
     */
    template < int dim > struct VectorDoFTuple
    {
	...
	case 2:
	  {
		...
		if (!constraints.is_constrained (dof_indices.dof_indices[0])
		    && constraints.can_store_line (dof_indices.
						   dof_indices[0]))
		  {
			...
			std::numeric_limits < double >::epsilon ())
		      constraints.set_inhomogeneity (dof_indices.
						     dof_indices[0],
						     inhomogeneity /
						     constraining_vector[0]);
		  }
    ...
    template < int dim >
      void
      add_tangentiality_constraints (const VectorDoFTuple < dim >
				     &dof_indices, const Tensor < 1,
				     dim > &tangent_vector,
				     AffineConstraints < double >&constraints,
  ...
  namespace internals
  {
    ...
    {
      const double tol =
	0.5 * cell->face (face)->line (line)->diameter () /
	cell->get_fe ().degree;
      const unsigned int dim = 3;
      const unsigned int spacedim = 3;

        hp_fe_values.reinit (cell,
			     (cell->active_fe_index () * GeometryInfo <
			      dim >::faces_per_cell + face) * GeometryInfo <
			     dim >::lines_per_face + line);
	...
	fe_values.get_quadrature_points ();
        ...
        std::vector < Tensor < 1,
	dim >> tangentials (fe_values.n_quadrature_points);
        std::vector < Vector < double >>values (fe_values.n_quadrature_points,
						Vector <
						double >(fe.n_components ()));
	  ...
	  for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
	    if (((dynamic_cast < const FESystem < dim > *>(&fe) != nullptr) &&
		 ...
		 && (fe.base_element (base_indices.first).
		     face_to_cell_index (line * fe.degree,
					 face) <=
		     fe.system_to_base_index (fe.
					      face_to_cell_index (i,
								  face)).
		     second)
		 && (fe.
		     system_to_base_index (fe.face_to_cell_index (i, face)).
		     second <=
		     fe.base_element (base_indices.first).
		     face_to_cell_index ((line + 1) * fe.degree - 1, face)))
		||
		((dynamic_cast < const FE_Nedelec < dim > *>(&fe) !=
		  nullptr) &&(line * fe.degree <= i)
		 && (i < (line + 1) * fe.degree)))
	      {
	...
	case 2:
	  {
	    ...
	    // of the face.
	    const unsigned int
	      face_coordinate_direction[GeometryInfo <
					dim >::faces_per_cell] = {
	      1, 1, 0, 0
	    };
	    const FEValuesExtractors::Vector vec (first_vector_component);
		...
		for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
		  if (((dynamic_cast < const FESystem < dim > *>(&fe) !=
			...
			(values[q_point] (first_vector_component) *
			 ...
			 tangentials[q_point][1]) *
			(fe_values[vec].
			 value (fe.face_to_cell_index (i, face),
	...
	case 3:
	  {
	    const FEValuesExtractors::Vector vec (first_vector_component);
	    FullMatrix < double >assembling_matrix (degree * fe.degree,
						    dim *
						    fe_values.
						    n_quadrature_points);
		...
		for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
		  if (((dynamic_cast < const FESystem < dim > *>(&fe) !=
			...
			first == base_indices)
		       && (fe.base_element (base_indices.first).
			   face_to_cell_index (2 * fe.degree,
					       face) <=
			   fe.system_to_base_index (fe.
						    face_to_cell_index (i,
									face)).
		...
		const double JxW = std::sqrt (fe_values.JxW (q_point) /
					      ((jacobians[q_point][0]
						[global_face_coordinate_directions
						 [face][0]] *
						jacobians[q_point][0]
						[global_face_coordinate_directions
						 [face][0]] +
		...
		for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
		  if (((dynamic_cast < const FESystem < dim > *>(&fe) !=
			...
			first == base_indices)
		       && (fe.base_element (base_indices.first).
			   face_to_cell_index (GeometryInfo <
					       dim >::lines_per_face *
					       fe.degree,
					       face) <=
			   fe.system_to_base_index (fe.
			   ...
			   second <
			   fe.base_element (base_indices.first).
			   face_to_cell_index ((degree + GeometryInfo <
						dim >::lines_per_face) *
					       fe.degree, face)))
		      ||
		      ((dynamic_cast < const FE_Nedelec < dim > *>(&fe) !=
			nullptr) &&(GeometryInfo <
				    dim >::lines_per_face * fe.degree <= i)
		       && (i <
			   (degree + GeometryInfo <
			    dim >::lines_per_face) * fe.degree)))
		    {
		      const Tensor < 1, dim > shape_value =
			(JxW *
			 fe_values[vec].value (fe.
					       face_to_cell_index (i, face),
					       q_point));
	    ...
	    // values.
	    {
	      ...
	      for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
		if (((dynamic_cast < const FESystem < dim > *>(&fe) !=
		      nullptr) &&(fe.system_to_base_index (fe.
							   face_to_cell_index
							   (i,
							    face)).first ==
				  base_indices)
  ...
  template < int dim >
    void
    project_boundary_values_curl_conforming (const DoFHandler < dim >
					     &dof_handler,
					     const unsigned int
					     first_vector_component,
					     const Function < dim >
					     &boundary_function,
					     const types::
					     boundary_id boundary_component,
					     AffineConstraints <
					     double >&constraints,
					     const Mapping < dim > &mapping)
  {
    ...
    for (unsigned int face = 0; face < GeometryInfo < dim >::faces_per_cell;
	 ++face)
      face_quadrature_collection.push_back (QProjector <
					    dim >::
					    project_to_face
		...
		if (cell->face (face)->boundary_id () == boundary_component)
		  {
			...
			*>(&cell->get_fe ()) == nullptr)
		    ...
		    // the edge.
		    internals::compute_face_projection_curl_conforming (cell,
									face,
									fe_face_values,
									boundary_function,
									first_vector_component,
		    ...
		    for (unsigned int dof = 0; dof < dofs_per_face; ++dof)
			  ...
			  && !(constraints.
			       is_constrained (face_dof_indices[dof])))
			{
			  constraints.add_line (face_dof_indices[dof]);

			  if (std::abs (dof_values[dof]) > 1e-13)
			    constraints.
			      set_inhomogeneity (face_dof_indices[dof],
						 dof_values[dof]);
			}
		  }
	...
	{
	  const QGauss < dim - 2 >
	    reference_edge_quadrature (2 * superdegree);
	  ...
	  for (unsigned int face = 0;
	    ...
	    for (unsigned int line = 0;
		 line < GeometryInfo < dim >::lines_per_face; ++line)
	      edge_quadrature_collection.push_back (QProjector <
						    dim >::
						    project_to_face
						    (QProjector <
						     dim -
			...
			*>(&cell->get_fe ()) == nullptr)
		    ...
		    for (unsigned int line = 0;
			 line < GeometryInfo < 3 >::lines_per_face; ++line)
		      internals::compute_edge_projection (cell,
							  face,
							  line,
							  fe_edge_values,
							  boundary_function,
		    ...
		    if (degree > 0)
		      internals::
			compute_face_projection_curl_conforming (cell, face,
								 fe_face_values,
								 boundary_function,
								 first_vector_component,
								 dof_values,
  ...
  template < int dim >
    void
    project_boundary_values_curl_conforming (const hp::DoFHandler < dim >
					     &dof_handler,
					     const unsigned int
					     first_vector_component,
					     const Function < dim >
  ...
  namespace internals
  {
	  ...
	  nullptr)
	{
      ...
      std::vector <
	unsigned int >associated_edge_dof_to_face_dof (degree + 1);

      // Lowest DoF in the base element allowed for this edge:
      const unsigned int lower_bound =
	fe.base_element (base_indices.first).face_to_cell_index (line *
								 (degree + 1),
								 face);
      // Highest DoF in the base element allowed for this edge:
      const unsigned int upper_bound =
	fe.base_element (base_indices.first).face_to_cell_index ((line + 1) *
								 (degree +
								  1) - 1,
								 face);
      ...
      for (unsigned int line_idx = 0; line_idx < fe.dofs_per_line; ++line_idx)
	{
	  ...
	  if (((dynamic_cast < const FESystem < dim > *>(&fe) != nullptr) &&
	      ...
	      (((dynamic_cast < const FE_Nedelec < dim > *>(&fe) !=
		 nullptr) ||(dynamic_cast < const FE_NedelecSZ < dim >
			     *>(&fe) != nullptr))
	   ...
	   ++q_point)
	{
	  ...
	  for (unsigned int j = 0; j < associated_edge_dofs; ++j)
	      ...
	      for (unsigned int i = 0; i < associated_edge_dofs; ++i)
		{
		  ...
		  edge_matrix (i, j) +=
		    fe_values.JxW (q_point) *
		    (fe_values[vec].value (i_cell_idx, q_point) *
		     tangential) * (fe_values[vec].value (j_cell_idx,
							  q_point) *
				    tangential);
		}
	...
	fe_face_values.get_quadrature_points ();
      ...
      std::vector < Vector <
	double >>values (fe_face_values.n_quadrature_points,
	...
	case 2:
	  ...
	  //       and contains a lot of overlap with that function.
	  {
		...
		const unsigned int cell_idx =
		  fe.face_to_cell_index (face_idx, face);
		if (((dynamic_cast < const FESystem < dim > *>(&fe) !=
		      nullptr) &&(fe.system_to_base_index (cell_idx).first ==
				  base_indices))
		    || (dynamic_cast < const FE_Nedelec < dim > *>(&fe) !=
			nullptr) ||(dynamic_cast < const FE_NedelecSZ < dim >
				    *>(&fe) != nullptr))
	...
	case 3:
	  {
	    ...
	    std::vector < std::vector <
	      unsigned int >>associated_edge_dof_to_face_dof (lines_per_face,
							      std::vector <
							      unsigned int
							      >(degree + 1));
	    ...
	    std::vector <
	      unsigned int >associated_face_dof_to_face_dof (2 * degree *
							     (degree + 1));
		...
		for (unsigned int j = 0; j < associated_face_dofs; ++j)
		  {
			...
			const unsigned int cell_i =
			  fe.face_to_cell_index (i_face_idx, face);
			cross_product_i = cross_product_3d (normal_vector,
							    fe_face_values
							    [vec].
							    value (cell_i,
								   q_point));
    ...
    template < int dim, typename DoFHandlerType >
      void
      compute_project_boundary_values_curl_conforming_l2 (const DoFHandlerType
							  & dof_handler,
							  const unsigned int
							  first_vector_component,
							  const Function <
	...
	{
	  const QGauss < dim - 1 >
	    reference_face_quadrature (2 * fe_collection[i].degree + 1);
			...
			if (cell->face (face)->boundary_id () ==
			    boundary_component)
			  {
				...
				*>(&cell->get_fe ()) == nullptr)
			    ...
			    compute_face_projection_curl_conforming_l2 (cell,
									face,
									fe_face_values,
									boundary_function,
									first_vector_component,
			    ...
			    face_dof_indices.resize (dofs_per_face);
			    cell->face (face)->
			      get_dof_indices (face_dof_indices,
					       cell->active_fe_index ());
				...
				if (dofs_processed[dof] &&
				    ...
				    && !(constraints.
					 is_constrained (face_dof_indices
							 [dof])))
				  {
				    constraints.
	...
	case 3:
	  {
	    ...
	    for (unsigned int i = 0; i < fe_collection.size (); ++i)
	      {
		const QGauss < dim - 2 >
		  reference_edge_quadrature (2 * fe_collection[i].degree + 1);
		for (unsigned int face = 0;
		     face < GeometryInfo < dim >::faces_per_cell; ++face)
		  {
		    for (unsigned int line = 0;
			    ...
			    boundary_component)
			  {
				...
				*>(&cell->get_fe ()) == nullptr)
			    ...
			    for (unsigned int line = 0;
				 ...
				 ++line)
			      {
				compute_edge_projection_l2 (cell,
							    face,
							    line,
							    fe_edge_values,
							    boundary_function,
			    ...
			    if (degree > 0)
			      {
				compute_face_projection_curl_conforming_l2
				  (cell, face, fe_face_values,
				   boundary_function, first_vector_component,
  ...
  template < int dim >
    void
    project_boundary_values_curl_conforming_l2 (const DoFHandler < dim >
						&dof_handler,
						const unsigned int
						first_vector_component,
						const Function < dim >
  ...
  template < int dim >
    void
    project_boundary_values_curl_conforming_l2 (const hp::DoFHandler < dim >
						&dof_handler,
						const unsigned int
						first_vector_component,
						const Function < dim >
  ...
  namespace internals
  {
	   ...
	   ++q_point)
	{
	  ...
	  tmp *=
	    fe_values.JxW (q_point) *
	    std::
	    sqrt (jacobians[q_point][0][face_coordinate_direction[face]] *
      ...
      std::vector < types::global_dof_index >
	face_dof_indices (fe.dofs_per_face);
	   ...
	   ++q_point)
	{
	  ...
	  tmp *=
	    fe_values.JxW (q_point) *
	    std::sqrt ((jacobians[q_point][0]
			[face_coordinate_directions[face][0]] *
			jacobians[q_point][0][face_coordinate_directions[face]
					      [0]] +
			jacobians[q_point][1][face_coordinate_directions[face]
					      [0]] *
			jacobians[q_point][1][face_coordinate_directions[face]
					      [0]] +
			jacobians[q_point][2][face_coordinate_directions[face]
					      [0]] *
			jacobians[q_point][2][face_coordinate_directions[face]
					      [0]]) *
		       (jacobians[q_point][0]
			[face_coordinate_directions[face][1]] *
			jacobians[q_point][0][face_coordinate_directions[face]
					      [1]] +
			jacobians[q_point][1][face_coordinate_directions[face]
					      [1]] *
  ...
  template < int dim >
    void
    project_boundary_values_div_conforming (const DoFHandler < dim >
					    &dof_handler,
					    const unsigned int
					    first_vector_component,
					    const Function < dim >
					    ...
					    const Mapping < dim > &mapping)
  {
    ...
    for (unsigned int face = 0; face < GeometryInfo < dim >::faces_per_cell;
	 ++face)
      quadrature_collection.push_back (QProjector <
				       dim >::
				       project_to_face (face_quadrature,
							face));

    hp::FEValues < dim > fe_values (mapping_collection,
				    fe_collection,
				    quadrature_collection, update_jacobians);
		...
		if (cell->face (face)->boundary_id () == boundary_component)
		  {
			...
			*>(&cell->get_fe ()) == nullptr)
		      {
			AssertThrow (dynamic_cast < const FE_RaviartThomas <
				     dim > *>(&cell->get_fe ()) != nullptr,
				     typename FiniteElement <
				     dim >::
				     ExcInterpolationNotImplemented ());
		      }
					    ...
					    dim > &mapping_collection)
  {
	...
	for (unsigned int face = 0;
	     face < GeometryInfo < dim >::faces_per_cell; ++face)
	  quadrature_collection.push_back (QProjector <
					   dim >::project_to_face (quadrature,
								   face));
      }
  ...
  template < int dim, int spacedim, template < int,
    int >class DoFHandlerType >
    void compute_no_normal_flux_constraints (const DoFHandlerType < dim,
					     spacedim > &dof_handler,
					     const unsigned int
					     first_vector_component,
					     const std::set <
  ...
  template < int dim, int spacedim, template < int,
    int >class DoFHandlerType >
    void compute_nonzero_normal_flux_constraints (const DoFHandlerType < dim,
						  spacedim > &dof_handler,
						  const unsigned int
						  first_vector_component,
						  const std::set <
						  ...
						  spacedim > &mapping)
  {
	...
	face_quadrature_collection.push_back (Quadrature < dim - 1 >
					      (unit_support_points));
      }
    ...
    using DoFToNormalsMap = std::multimap <
      ...
      typename DoFHandlerType < dim, spacedim >::active_cell_iterator >>;
    std::map < internal::VectorDoFTuple < dim >,
      Vector < double >>dof_vector_to_b_values;
	...
	for (unsigned int face_no = 0;
	     face_no < GeometryInfo < dim >::faces_per_cell; ++face_no)
	  if ((b_id =
	       boundary_ids.find (cell->face (face_no)->boundary_id ())) !=
	      boundary_ids.end ())
	    {
	      const FiniteElement < dim > &fe = cell->get_fe ();
		...
		if (fe.face_system_to_component_index (i).first ==
		    first_vector_component)
		  {
		    ...
		    Assert (first_vector_component + dim <=
			    fe.n_components (),
			    ExcMessage
		    ...
		    for (unsigned int d = 0; d < dim; ++d)
		      Assert (vector_dofs.dof_indices[d] <
			      dof_handler.n_dofs (), ExcInternalError ());
		    ...
		    Tensor < 1, dim > normal_vector =
		      (cell->face (face_no)->get_manifold ().
		    ...
		    // the map
		    dof_to_normals_map.insert (std::make_pair (vector_dofs,
							       std::
							       make_pair
							       (normal_vector,
								cell)));
		    dof_vector_to_b_values.
		      insert (std::make_pair (vector_dofs, b_values));
	...
	using CellToNormalsMap =
	  std::map < typename DoFHandlerType < dim,
	  spacedim >::active_cell_iterator, std::pair < Tensor < 1, dim >,
	  unsigned int >>;
	...
	for (typename CellToNormalsMap::const_iterator x =
	  ...
	  std::cout << "      " << x->first << " -> (" << x->second.
	    first << ',' << x->second.second << ')' << std::endl;
#endif
	...
	switch (max_n_contributions_per_cell)
	  ...
	  case 1:
		...
		normal_value += b_values[i] * normal[i];
	      internal::add_constraint (dof_indices,
					normal, constraints, normal_value);
	  ...
	  case dim:
		...
		dof_vector_to_b_values[dof_indices];
	      for (unsigned int i = 0; i < dim; ++i)
		if (!constraints.
		    is_constrained (same_dof_range[0]->first.dof_indices[i])
		    && constraints.can_store_line (same_dof_range[0]->first.
						   dof_indices[i]))
		  {
	  ...
	  default:
		...
		typename DoFHandlerType < dim,
		spacedim >::active_cell_iterator, std::list < Tensor < 1,
		dim >>>;
	      CellContributions cell_contributions;

	      for (typename DoFToNormalsMap::const_iterator q =
		   same_dof_range[0]; q != same_dof_range[1]; ++q)
		cell_contributions[q->second.second].push_back (q->second.
								first);
	      ...
	      for (typename CellContributions::const_iterator contribution =
		   ...
		   contribution != cell_contributions.end (); ++contribution)
		{
#ifdef DEBUG_NO_NORMAL_FLUX
		  std::cout
		    ...
		    << std::endl;
		  for (typename std::list < Tensor < 1,
		       dim >>::const_iterator t =
		       contribution->second.begin ();
		       t != contribution->second.end (); ++t)
		    std::cout << "      " << *t << std::endl;
#endif
		  ...
		  Tensor < 1, dim > normals[dim - 1];
		  {
		    ...
		    for (typename std::list < Tensor < 1,
			 dim >>::const_iterator t =
			 contribution->second.begin ();
			 t != contribution->second.end (); ++t, ++index)
		      normals[index] = *t;
  ...
  namespace internal
  {
    template < int dim > struct PointComparator
    {
      bool
  ...
  template < int dim, int spacedim, template < int,
    int >class DoFHandlerType >
    void compute_normal_flux_constraints (const DoFHandlerType < dim,
					  spacedim > &dof_handler,
					  const unsigned int
					  first_vector_component,
					  const std::set <
  ...
  template < int dim, int spacedim, template < int,
    int >class DoFHandlerType >
    void compute_nonzero_tangential_flux_constraints (const DoFHandlerType <
						      dim,
						      spacedim > &dof_handler,
						      const unsigned int
						      first_vector_component,
  ...
  {
    AffineConstraints <
      double >no_normal_flux_constraints (constraints.get_local_lines ());
    compute_nonzero_normal_flux_constraints (dof_handler,
					     first_vector_component,
					     boundary_ids, function_map,
    ...
    std::set < std::array < types::global_dof_index, dim >,
      internal::PointComparator < dim >> vector_dofs;
    std::vector < types::global_dof_index > face_dofs;

    std::map < std::array < types::global_dof_index, dim >,
      Vector < double >>dof_vector_to_b_values;

    std::set < types::boundary_id >::iterator b_id;
    std::vector < std::array < types::global_dof_index,
      dim >> cell_vector_dofs;
    for (typename DoFHandlerType < dim,
	 spacedim >::active_cell_iterator cell = dof_handler.begin_active ();
	 cell != dof_handler.end (); ++cell)
      if (!cell->is_artificial ())
	for (unsigned int face_no = 0;
	     face_no < GeometryInfo < dim >::faces_per_cell; ++face_no)
	  if ((b_id =
	      ...
	      for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
		{
		  if (fe.face_system_to_component_index (i).first >=
			...
			function_map.at (*b_id)->value (point, component);
		      dof_to_b_value.
			insert (std::make_pair (face_dofs[i], b_value));
		    }
		}
	      ...
	      for (unsigned int i = 0; i < n_scalar_indices; ++i)
		{
		  ...
		  dof_vector_to_b_values.
		    insert (std::make_pair (cell_vector_dofs[i], b_values));
		}
	    }
    ...
    for (typename std::set < std::array < types::global_dof_index, dim >,
	 ...
	 vector_dofs.begin (); it != vector_dofs.end (); ++it)
      {
	...
	if (n_constraints > 0)
	  {
	    ...
	    const std::vector < std::pair < types::global_dof_index,
	      double >>*constrained =
	      no_normal_flux_constraints.
	      get_constraint_entries ((*it)[constrained_index]);
  ...
  namespace internal
  {
    template < int dim, int spacedim, typename Number > struct IDScratchData
    {
      IDScratchData (const dealii::hp::MappingCollection < dim,
		     spacedim > &mapping,
	...
	spacedim >>> tmp_vector_gradients;
      ...
      Number >::IDScratchData (const dealii::hp::MappingCollection < dim,
			       spacedim > &mapping,
			       const dealii::hp::FECollection < dim,
			       spacedim > &fe,
			       const dealii::hp::QCollection < dim > &q,
			       const UpdateFlags
			       update_flags):x_fe_values (mapping, fe, q,
							  update_flags)
    {
    }
    ...
    template < int dim, int spacedim, typename Number >
      IDScratchData < dim, spacedim,
      Number >::IDScratchData (const IDScratchData & data):x_fe_values (data.
									x_fe_values.
    ...
    template < int dim, int spacedim, typename Number >
      void
      IDScratchData < dim, spacedim,
      Number >::resize_vectors (const unsigned int n_q_points,
				const unsigned int n_components)
    {
      function_values.resize (n_q_points, Vector < Number > (n_components));
      function_grads.resize (n_q_points, std::vector < Tensor < 1, spacedim,
			     Number >> (n_components));
      ...
      tmp_vector_gradients.resize (n_q_points, std::vector < Tensor < 1,
				   spacedim >> (n_components));
    }
      ...
      if (update_flags & update_values)
	{
	  ...
	  if (fe_is_system)
	    {
	      exact_solution.vector_value_list (fe_values.
						get_quadrature_points (),
      ...
      if (update_flags & update_gradients)
	{
	  ...
	  if (fe_is_system)
	    {
	      exact_solution.vector_gradient_list (fe_values.
						   get_quadrature_points (),
	    ...
	    {
	      exact_solution.gradient_list (fe_values.
					    get_quadrature_points (),
					    data.tmp_gradients);
	      for (unsigned int i = 0; i < n_q_points; ++i)
	  ...
	  if (update_flags & update_normal_vectors)
		...
		{
		  // compute (f.n) n
		  const typename ProductType < Number,
		    double >::type f_dot_n =
	...
	case W1p_norm:
	  ...
	  for (unsigned int q = 0; q < n_q_points; ++q)
		...
		if (data.weight_vectors[q] (k) != 0)
		  sum +=
		    std::pow (static_cast <
			      double >(numbers::NumberTraits <
				       Number >::abs_square (data.
							     psi_values[q]
							     (k))),
	  ...
	  for (unsigned int q = 0; q < n_q_points; ++q)
		...
		if (data.weight_vectors[q] (k) != 0)
		  sum +=
		    ...
		    Number >::abs_square (data.psi_values[q] (k)) *
    ...
    template < int dim,
      ...
      int spacedim >
      static void
      do_integrate_difference (const dealii::hp::MappingCollection < dim,
			       spacedim > &mapping,
	...
	dof.get_fe_collection ();
      ...
      for (typename DoFHandlerType::active_cell_iterator cell =
	   dof.begin_active (); cell != dof.end (); ++cell)
	if (cell->is_locally_owned ())
	  {
	    ...
	    difference (cell->active_cell_index ()) =
	      integrate_difference_inner < dim, spacedim,
	      Number > (exact_solution, norm, weight, update_flags, exponent,
			n_components, data);
	  }
	else
  ...
  template < int dim, class InVector, class OutVector, int spacedim >
    ...
    integrate_difference (const Mapping < dim, spacedim > &mapping,
			  ...
			  const double exponent)
  {
    internal::do_integrate_difference (hp::MappingCollection < dim,
				       spacedim > (mapping), dof, fe_function,
				       exact_solution, difference,
				       hp::QCollection < dim > (q), norm,
				       weight, exponent);
  }
  ...
  template < int dim, class InVector, class OutVector, int spacedim >
    ...
    integrate_difference (const DoFHandler < dim, spacedim > &dof,
			  ...
			  const double exponent)
  {
    internal::do_integrate_difference (hp::StaticMappingQ1 < dim,
				       spacedim >::mapping_collection, dof,
				       fe_function, exact_solution,
				       difference,
				       hp::QCollection < dim > (q), norm,
				       weight, exponent);
  }
  ...
  template < int dim, class InVector, class OutVector, int spacedim >
    void
    integrate_difference (const dealii::hp::MappingCollection < dim,
			  spacedim > &mapping,
			  ...
			  const double exponent)
  {
    internal::do_integrate_difference (hp::MappingCollection < dim,
				       spacedim > (mapping), dof, fe_function,
				       exact_solution, difference, q, norm,
				       weight, exponent);
  }
  ...
  template < int dim, int spacedim, class InVector >
    ...
    compute_global_error (const Triangulation < dim, spacedim > &tria,
			  ...
			  const NormType & norm, const double exponent)
  {
		      ...
		      const Point < spacedim > &point)
  {
    ...
    Assert (GeometryInfo < dim >::distance_to_unit_cell (cell_point.second) <
	    1e-10, ExcInternalError ());

    const Quadrature < dim > quadrature (GeometryInfo <
					 dim >::
					 project_to_unit_cell (cell_point.
							       second));
  ...
  {
    point_value (StaticMappingQ1 < dim, spacedim >::mapping, dof, fe_function,
		 point, value);
  }
		 ...
		 const Point < spacedim > &point)
  {
    return point_value (hp::StaticMappingQ1 < dim,
			spacedim >::mapping_collection, dof, fe_function,
			point);
  }
		 ...
		 Vector < typename VectorType::value_type > &value)
  {
    ...
    const Quadrature < dim > quadrature (GeometryInfo <
					 ...
					 project_to_unit_cell (cell_point.
							       second));
    hp::FEValues < dim, spacedim > hp_fe_values (mapping, fe,
						 hp::QCollection < dim >
						 (quadrature), update_values);
  ...
  template < int dim, typename VectorType, int spacedim >
    ...
    point_gradient (const DoFHandler < dim, spacedim > &dof,
		    ...
		    const Point < spacedim > &point,
		    std::vector < Tensor < 1, spacedim,
		    typename VectorType::value_type >> &gradients)
  {
    point_gradient (StaticMappingQ1 < dim, spacedim >::mapping,
		    dof, fe_function, point, gradients);
  }
  ...
  template < int dim, typename VectorType, int spacedim >
    ...
    point_gradient (const hp::DoFHandler < dim, spacedim > &dof,
		    ...
		    const Point < spacedim > &point)
  {
    return point_gradient (hp::StaticMappingQ1 < dim,
			   spacedim >::mapping_collection, dof, fe_function,
			   point);
  }
  ...
  template < int dim, typename VectorType, int spacedim >
    ...
    point_gradient (const Mapping < dim, spacedim > &mapping,
		    ...
		    const Point < spacedim > &point,
		    std::vector < Tensor < 1, spacedim,
		    typename VectorType::value_type >> &gradient)
  {
    const FiniteElement < dim > &fe = dof.get_fe ();
    ...
    // the given fe_function at this point
    using Number = typename VectorType::value_type;
    std::vector < std::vector < Tensor < 1, dim, Number >>> u_gradient (1,
									std::
									vector
									<
									Tensor
		    ...
		    const Point < spacedim > &point)
  {
    ...
    std::vector < Tensor < 1, dim,
      typename VectorType::value_type >> gradient (1);
  ...
  namespace internal
  {
    template < typename VectorType >
      typename std::enable_if < dealii::is_serial_vector <
      VectorType >::value == true >::type subtract_mean_value (VectorType & v,
							       const std::
							       vector < bool >
							       &p_select)
    {
    ...
    template < typename VectorType >
      typename std::enable_if < dealii::is_serial_vector <
      VectorType >::value ==
      false >::type subtract_mean_value (VectorType & v,
  ...
  namespace internal
  {
    ...
    template < typename Type >
      ...
      set_possibly_complex_number (const double r,
				   const double i, std::complex < Type > &n)
    {
  ...
  template < int dim, typename VectorType, int spacedim >
    ...
    compute_mean_value (const Mapping < dim, spacedim > &mapping,
			const DoFHandler < dim, spacedim > &dof,
			const Quadrature < dim > &quadrature,
			const VectorType & v, const unsigned int component)
  {
    using Number = typename VectorType::value_type;
	...
	dynamic_cast < const parallel::Triangulation < dim,
	spacedim > *>(&dof.get_triangulation ()))
      {
	...
	double my_values[3] =
	  { mean_double.real (), mean_double.imag (), area };
  ...
  template < int dim,
    ...
    get_position_vector (const DoFHandlerType < dim, spacedim > &dh,
			 VectorType & vector, const ComponentMask & mask)
  {
    ...
    const ComponentMask fe_mask (mask.size ()? mask :
				 ComponentMask (fe.get_nonzero_components (0).
						size (), true));
	...
	FEValues < dim, spacedim > fe_v (map_q, fe, quad,
					 update_quadrature_points);
	...
	Assert (fe.is_primitive (),
	  ...
	  if (cell->is_locally_owned ())
	      ...
	      for (unsigned int q = 0; q < points.size (); ++q)
		{
		  ...
		  if (fe_mask[comp])
		    ::dealii::internal::ElementAccess <
		      VectorType >::set (points[q][fe_to_real[comp]], dofs[q],
					 vector);
		}
	    }
      }
