...
namespace ResFlow
{
  ...
  {
  public:
    FluxBoundaryValues ():Function < dim > (dim)
    {
    }
  ...
  template < int dim >
    void FluxBoundaryValues < dim >::vector_value (const Point < dim > &p,
						   Vector <
						   double >&bdry_flux) const
  {
    ...
    const double beta = 1;
      ...
      bdry_flux (0) =
      alpha * p[1] * p[1] / 2 + beta - alpha * p[0] * p[0] / 2;
      bdry_flux (1) = alpha * p[0] * p[1];
  }

  template < int dim > class ResFlowProblem
  {
  public:
  ...
  template < int dim >
    ResFlowProblem <
    dim >::ResFlowProblem (const unsigned int degree):degree (degree),
    mpi_communicator (MPI_COMM_WORLD), fe (FE_RaviartThomas < dim > (degree),
					   1, FE_DGQ < dim > (degree), 1),
    triangulation (mpi_communicator), dof_handler (triangulation),
    pcout (std::cout,
	   (Utilities::MPI::this_mpi_process (mpi_communicator) == 0))
  {
  }

  template < int dim > ResFlowProblem < dim >::~ResFlowProblem ()
  {
    dof_handler.clear ();
  }

  template < int dim > void ResFlowProblem < dim >::make_grid ()
  {
    unsigned int nx = 1;
  ...
  template < int dim > void ResFlowProblem < dim >::setup_system ()
  {
    ...
    pcout << "Number of Owned dofs: " << dof_handler.n_locally_owned_dofs ()
      ...
      << owned_partitioning[1].n_elements () << ')' << std::endl << std::endl;
    ...
    relevant_partitioning[1] = locally_relevant_dofs.get_view (n_u,
							       n_u + n_p);

    pcout << "Number of Relevant dofs: "
  ...
  template < int dim > void ResFlowProblem < dim >::run ()
  {
    pcout << "Running with "
